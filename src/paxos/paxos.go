...
func NewReplica(id int, peerAddrList []string, thrifty bool, exec bool, dreply bool, durable bool, slow bool) *Replica {
...
var mprerep genericsmr.Message

func (r *Replica) replyPrepare(replicaId int32, reply *paxosproto.PrepareReply) {
	//r.SendMsg(replicaId, r.prepareReplyRPC, reply)
	mprerep.PeerId = replicaId
	mprerep.Code = r.prepareReplyRPC
	mprerep.Msg = reply
	r.MessageChan <- mprerep
}

var maccrpe genericsmr.Message

func (r *Replica) replyAccept(replicaId int32, reply *paxosproto.AcceptReply) {
	//r.SendMsg(replicaId, r.acceptReplyRPC, reply)
	maccrpe.PeerId = replicaId
	maccrpe.Code = r.acceptReplyRPC
	maccrpe.Msg = reply
	r.MessageChan <- maccrpe
}
...
var mpc genericsmr.Message
var mpcs genericsmr.Message
			...
			if r.instanceSpace[i].cmds != nil {
				...
				for j := 0; j < len(inst.cmds); j++ {
					...
					if r.Dreply && inst.lb != nil && inst.lb.clientProposals != nil {
						//if inst.lb != nil && inst.lb.clientProposals != nil && inst.cmds[j].Op == state.GET {
						propreply := &genericsmrproto.ProposeReplyTS{
							TRUE,
