...
var doNew *bool = flag.Bool("n", false, "Use the new model as the replication protocol. Defaults to false.")
...
var slow = flag.Bool("slow", false, "Let the server be slow. Defaults to be false.")
...
func main() {
	...
	if *doEpaxos {
		...
		rep := epaxos.NewReplica(replicaId, nodeList, *thrifty, *exec, *dreply, *beacon, *durable, *slow)
		rpc.Register(rep)
	} else if *doMencius {
		log.Println("Starting Mencius replica...")
		rep := mencius.NewReplica(replicaId, nodeList, *thrifty, *exec, *dreply, *durable, *slow)
		rpc.Register(rep)
		//} else if *doGpaxos {
		...
		log.Println("Starting new model replica...")
		rep := newmodel.NewReplica(replicaId, nodeList, *thrifty, *exec, *dreply, *beacon, *durable, *slow)
		rpc.Register(rep)
	} else {
		log.Println("Starting classic Paxos replica...")
		rep := paxos.NewReplica(replicaId, nodeList, *thrifty, *exec, *dreply, *durable, *slow)
		rpc.Register(rep)
	}
