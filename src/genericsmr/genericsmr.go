...
type Replica struct {
	...
	ReplyMessageChan chan ReplyMessage
...
func NewReplica(id int, peerAddrList []string, thrifty bool, exec bool, dreply bool) *Replica {
	r := &Replica{
		...
		make(chan ReplyMessage, CHAN_BUFFER_SIZE),
		state.InitState(),
...
func (r *Replica) WaitForClientConnections() {
	for !r.Shutdown {
		...
		//dlog.Printf("Connection from %v", conn.RemoteAddr.String())
...
func (r *Replica) clientListener(conn net.Conn) {
	...
	for !r.Shutdown && err == nil {
		...
		if msgType, err = reader.ReadByte(); err != nil {
			log.Println("Error: Read msgType err", err)
			break
		}
		...
		case genericsmrproto.PROPOSE:
			...
			if err = prop.Unmarshal(reader); err != nil {
				log.Println("Error: Unmarshal prop err")
				break
			}
...
func (r *Replica) SendMessage() {
	...
	for !r.Shutdown {
		...
		if message.NoFlush {
			r.SendMsgNoFlush(message.PeerId, message.Code, message.Msg)
		} else {
			r.SendMsg(message.PeerId, message.Code, message.Msg)
		}
	}
}

func (r *Replica) SendReply() {
	var messageReply ReplyMessage
	for !r.Shutdown {
		messageReply = <-r.ReplyMessageChan
		r.ReplyProposeTS(messageReply.Reply, messageReply.Writer)
	}
}
