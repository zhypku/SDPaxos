...
var mrprepare genericsmr.Message

func (r *Replica) replyPrepare(replicaId int32, reply *epaxosproto.PrepareReply) {
	//r.SendMsg(replicaId, r.prepareReplyRPC, reply)
	mrprepare.PeerId = replicaId
	mrprepare.Code = r.prepareReplyRPC
	mrprepare.Msg = reply
	r.MessageChan <- mrprepare
}

var mrpreacc genericsmr.Message
...
func (r *Replica) replyPreAccept(replicaId int32, reply *epaxosproto.PreAcceptReply) {
	...
	mrpreacc.PeerId = replicaId
	mrpreacc.Code = r.preAcceptReplyRPC
	mrpreacc.Msg = reply
	r.MessageChan <- mrpreacc
}

var mracc genericsmr.Message

func (r *Replica) replyAccept(replicaId int32, reply *epaxosproto.AcceptReply) {
	//r.SendMsg(replicaId, r.acceptReplyRPC, reply)
	mracc.PeerId = replicaId
	mracc.Code = r.acceptReplyRPC
	mracc.Msg = reply
	r.MessageChan <- mracc
}

var mrtpreacc genericsmr.Message
...
func (r *Replica) replyTryPreAccept(replicaId int32, reply *epaxosproto.TryPreAcceptReply) {
	...
	mrtpreacc.PeerId = replicaId
	mrtpreacc.Code = r.tryPreAcceptReplyRPC
	mrtpreacc.Msg = reply
	r.MessageChan <- mrtpreacc
}

var mprepare genericsmr.Message
...
func (r *Replica) bcastPrepare(replica int32, instance int32, ballot int32) {
	...
	for sent := 0; sent < n; {
		...
		mprepare.PeerId = q
		r.MessageChan <- mprepare
		sent++
	}
}

var mpa genericsmr.Message
...
func (r *Replica) bcastPreAccept(replica int32, instance int32, ballot int32, cmds []state.Command, seq int32, deps [DS]int32) {
	...
	for q := 0; q < r.N-1; q++ {
		...
		mpa.PeerId = r.PreferredPeerOrder[q]
		r.MessageChan <- mpa
		sent++
		if sent >= n {
			break
...
func (r *Replica) bcastAccept(replica int32, instance int32, ballot int32, count int32, seq int32, deps [DS]int32) {
	...
	for q := 0; q < r.N-1; q++ {
		...
		mea.PeerId = r.PreferredPeerOrder[q]
		r.MessageChan <- mea
		sent++
		if sent >= n {
			break
...
var mec genericsmr.Message
var mecs genericsmr.Message
...
func (r *Replica) bcastCommit(replica int32, instance int32, cmds []state.Command, seq int32, deps [DS]int32) {
	...
	for q := 0; q < r.N-1; q++ {
		...
		if r.Thrifty && sent >= r.N/2 {
			...
			mec.PeerId = r.PreferredPeerOrder[q]
			r.MessageChan <- mec
		} else {
			//r.SendMsg(r.PreferredPeerOrder[q], r.commitShortRPC, argsShort)
			mecs.PeerId = r.PreferredPeerOrder[q]
			r.MessageChan <- mecs
			sent++
		}
	}
...
func (r *Replica) handlePreAccept(preAccept *epaxosproto.PreAccept) {
	...
	if changed || uncommittedDeps || preAccept.Replica != preAccept.LeaderId || !isInitialBallot(preAccept.Ballot) {
		...
		r.MessageChan <- genericsmr.Message{preAccept.LeaderId, r.preAcceptOKRPC, pok, false}
	}
